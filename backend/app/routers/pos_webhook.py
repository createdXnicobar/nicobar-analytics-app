...
async def pos_webhook(
    ...
    try:
        ...
        for line in lines:
            ...
            logger.debug(f"Processing line: Order {line.Order_No}, Line {line.Line_No}, SKU {line.Item_Code}")
            
            exists = await purchase_events().find_one({"idemKey": idem_key})
            if exists:
                skipped += 1
                logger.debug(f"Skipping duplicate line with key: {idem_key}")
                continue

            snap = await fetch_product_by_sku(line.Item_Code)
            if snap:
                logger.debug(f"Product snapshot found for SKU {line.Item_Code}: {snap.title}")
            else:
                logger.warning(f"No product snapshot found for SKU: {line.Item_Code}")
                
            # These conversions should now be safe since we validated above
            price_list, _ = _safe_to_float(line.Price, "Price", f"Order {line.Order_No}, Line {line.Line_No}")
            price_billed, _ = _safe_to_float(line.Billed_Price, "Billed_Price", f"Order {line.Order_No}, Line {line.Line_No}")
            qty, _ = _safe_to_int(line.Quantity, "Quantity", f"Order {line.Order_No}, Line {line.Line_No}")
            ...
            try:
                ...
                # This should not happen due to validation above, but log and skip this line
            ...
            created += 1
            logger.debug(f"Created purchase event for order {line.Order_No}, line {line.Line_No}")
